...
export async function GET(req: Request) {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INVALID_FILE',
          message: 'Valid .xml file parameter is required.',
          details: null,
        },
      },
      { status: 400 }
  ...
  try {
    ...
    return NextResponse.json(
      { success: true, data: { knowledgeBase, userInstructionsHtml }, error: null },
      { status: 200 }
    );
  } catch (err: any) {
      ...
      return NextResponse.json(
        {
          ...
          error: {
            code: 'FILE_NOT_FOUND',
            message: `Knowledge base file not found: ${file}`,
            details: err,
          },
        },
        { status: 404 }
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message: `An error occurred while retrieving file: ${file}`,
