...
/**
 * GET /api/get-mba-knowledge-base
 * Retrieves content and user instructions for a specific MBA knowledge base XML file.
 * Query param: `fileName` (e.g., 'some-mba-document.xml')
 * Response format: { success, data, error }
 */
export async function GET(request: Request) {
  const apiRouteName = 'get-mba-knowledge-base';

  // Get the filename from the URL
  const url = new URL(request.url);
  const fileName = url.searchParams.get('fileName') || url.searchParams.get('file'); // Support both parameter names for backward compatibility
  ...
  // MBA knowledge base files are expected directly in 'ai_instructions'
  let filePath = path.join(baseDir, 'ai_instructions', fileName);

  // Add debugging to list all files in the directory
  try {
    const mbaDir = path.join(baseDir, 'ai_instructions');

    try {
      await fsPromises.access(mbaDir);

      const files = await fsPromises.readdir(mbaDir);
      ...
      if (!fileExists) {
        ...
        if (closestMatch) {
          ...
          filePath = path.join(baseDir, 'ai_instructions', closestMatch);
        } else {
        ...
        return NextResponse.json(
          {
            ...
            error: {
              code: 'DIR_NOT_FOUND',
              message: `MBA directory not found at ${mbaDir} or alternative paths.`,
              details: { searchedPaths: [mbaDir, ...altPaths] },
            },
          },
          { status: 404 }
        );
    ...
    // Error in directory listing, will try alternative paths
  }

  // Dynamically construct user instructions path for MBA
  ...
  try {
    ...
    try {
      ...
      // If user instructions file is not found, try alternative paths
      const userInstrAltPaths = [
        path.join('/app', 'user_instructions', 'mba', userInstructionHtmlFile),
        path.join(process.cwd(), 'user_instructions', 'mba', userInstructionHtmlFile),
        path.join(process.cwd(), 'app', 'user_instructions', 'mba', userInstructionHtmlFile),
      ];
